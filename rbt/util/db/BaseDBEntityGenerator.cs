...
{
    public abstract class BaseDBEntityGenerator
    {
        public abstract IList<TableInfo> GetTableInfo(
            string schema,
            ...
            )
        {
            ...
            //取得Table 資訊
            //================================
            var tableInfoList = this.GetTableInfo(schema, SpecifiedTable);

            //================================
            //逐筆處理 Table
            //================================
            foreach (TableInfo table in tableInfoList)
            {
                //=======================================
                //取得 table 資料
                //=======================================
                ...
                //不存在column資料時跳過
                if (table.ColumnInfoDic.Count < 1)
                {
                    continue;
                }

                //table 註解
                var tableComments = prepareStr(
                    replaceBreakLine(table.TABLE_COMMENT)
                ...
                foreach (ColumnInfo clumnInfo in table.ColumnInfoDic.Values)
                {
                    //=====================================================
                    // 取得欄位參數
                    //=====================================================
                    //"COLUMN_NAME"
                    string columnName = prepareStr(clumnInfo.COLUMN_NAME);
                    //"DATA_TYPE"
                    string dataType = StringUtil.SafeTrim(clumnInfo.DATA_TYPE);
                    //"COMMENTS"
                    string orgComments = replaceBreakLine(clumnInfo.COLUMN_COMMENT);
                    string comments = prepareStr(orgComments);
                    //DATA_LENGTH
                    int dataLenght = clumnInfo.DATA_LENGTH;
